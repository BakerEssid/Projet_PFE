...
CAN_FilterTypeDef Filtercan1;   // Configuration du filtre CAN
CAN_RxHeaderTypeDef pRxHeader;  // En-tête de la trame reçue
CAN_TxHeaderTypeDef pTxHeader;  // En-tête de la trame à transmettre
uint8_t aTxData[8];             // Données à envoyer
uint8_t aRxData[8];             // Données reçues
uint8_t test[8];                // Tableau pour stocker les données reçues
char Datacheck = 0;             // Flag de réception
...
int main(void)
{
  ...
  while (1)
  {
	  // Données capteurs (ID = 0x001 → test[6] == 0x41)
	  if (test[6]==0x41)
	  {
		 ...
		 // Commande de ventilateur selon la température
		 if (test[2]>30)
		 {
		 ...
		 // Contrôle direction d essuie glace
		 if (test[3] == 0)
		 {
		 ...
		 // Indicateur de régime moteur (RPM)
		 if (test[4]>150)
		 {
	  ...
	  //Cas 2 : Commande manuelle (ID = 0x003 → test[6] == 0x4D)
	  if (test[6]==0x4D)
	  {
		  if (test[5]==0x30)
		  {
	  ...
	  // Envoi d’une trame de retour sur le bus CAN
